...
plugin_name = "iwhat.bobplugin"

def update_appcast(message):
    with open ("src/info.json", "r") as f:
        info = json.load(f)
    version = info["version"]
    release_file = Path(f"release/{plugin_name}")
    ...
    version_info = {
        ...
        "desc": message,
        "sha256": file_hash,
        "url": f"{info['homepage']}/releases/download/v{version}/{plugin_name}",
        "minBobVersion": "0.5.0"
    }
    ...
    if appcast_file.is_file():
        ...
        appcast = dict(identifier=info["identifier"], versions=[])
